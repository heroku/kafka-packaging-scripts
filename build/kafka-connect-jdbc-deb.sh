...
if [ "x$SIGN" == "xyes" ]; then
    sudo --login debsign `readlink -f confluent-kafka-connect-jdbc_*.changes`
fi
cp confluent-kafka-connect-jdbc_*.build confluent-kafka-connect-jdbc_*.changes confluent-kafka-connect-jdbc_*.tar.gz confluent-kafka-connect-jdbc_*.dsc  confluent-kafka-connect-jdbc_*.deb /vagrant/output/
rm -rf /tmp/confluent
