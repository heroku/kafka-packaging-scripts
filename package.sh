...
vagrant ssh rpm -- sudo VERSION=$KAFKA_VERSION REVISION=$REVISION BRANCH=$KAFKA_BRANCH "SCALA_VERSIONS=\"$SCALA_VERSIONS\"" "PS_ENABLED=\"$PS_ENABLED\"" "PS_PACKAGES=\"$PS_PACKAGES\"" "PS_CLIENT_PACKAGE=\"$PS_CLIENT_PACKAGE\"" "CONFLUENT_VERSION=\"$CONFLUENT_VERSION\"" SKIP_TESTS=$KAFKA_BUILD_SKIP_TESTS /tmp/kafka-archive.sh
vagrant ssh rpm -- cp /vagrant/build/kafka-rpm.sh /tmp/kafka-rpm.sh
vagrant ssh rpm -- -t sudo VERSION=$KAFKA_VERSION REVISION=$REVISION BRANCH=$KAFKA_BRANCH "SCALA_VERSIONS=\"$SCALA_VERSIONS\"" "PS_ENABLED=\"$PS_ENABLED\"" "PS_PACKAGES=\"$PS_PACKAGES\"" "PS_CLIENT_PACKAGE=\"$PS_CLIENT_PACKAGE\"" "CONFLUENT_VERSION=\"$CONFLUENT_VERSION\"" SKIP_TESTS=$KAFKA_BUILD_SKIP_TESTS SIGN=$SIGN /tmp/kafka-rpm.sh
...
vagrant ssh rpm -- -t sudo VERSION=$CONFLUENT_VERSION RPM_VERSION=$RPM_VERSION RPM_RELEASE=$RPM_RELEASE RPM_RELEASE_POSTFIX=$RPM_RELEASE_POSTFIX "SCALA_VERSIONS=\"$SCALA_VERSIONS\"" KAFKA_RPM_VERSION=$KAFKA_RPM_VERSION LIBRDKAFKA_RPM_VERSION=$LIBRDKAFKA_RPM_VERSION SIGN=$SIGN /tmp/platform-rpm.sh
