...
vagrant ssh rpm -- sudo VERSION=$KAFKA_VERSION REVISION=$REVISION BRANCH=$KAFKA_BRANCH "SCALA_VERSIONS=\"$SCALA_VERSIONS\"" "PS_PACKAGES=\"$PS_PACKAGES\"" "PS_CLIENT_PACKAGE=\"$PS_CLIENT_PACKAGE\"" "CONFLUENT_VERSION=\"$CONFLUENT_VERSION\"" /tmp/kafka-archive.sh
vagrant ssh rpm -- cp /vagrant/build/kafka-rpm.sh /tmp/kafka-rpm.sh
vagrant ssh rpm -- -t sudo VERSION=$KAFKA_VERSION REVISION=$REVISION BRANCH=$KAFKA_BRANCH "SCALA_VERSIONS=\"$SCALA_VERSIONS\"" "PS_PACKAGES=\"$PS_PACKAGES\"" "PS_CLIENT_PACKAGE=\"$PS_CLIENT_PACKAGE\"" "CONFLUENT_VERSION=\"$CONFLUENT_VERSION\"" SIGN=$SIGN /tmp/kafka-rpm.sh
vagrant ssh deb -- cp /vagrant/build/kafka-deb.sh /tmp/kafka-deb.sh
vagrant ssh deb -- -t sudo VERSION=$KAFKA_VERSION REVISION=$REVISION BRANCH=$KAFKA_BRANCH "SCALA_VERSIONS=\"$SCALA_VERSIONS\"" "PS_PACKAGES=\"$PS_PACKAGES\"" "PS_CLIENT_PACKAGE=\"$PS_CLIENT_PACKAGE\"" "CONFLUENT_VERSION=\"$CONFLUENT_VERSION\"" SIGN=$SIGN /tmp/kafka-deb.sh
